...
module.exports =
	new DDCommand()
		.setName("sparebot")
		.setDescription("Make someone no longer the DD bot.")
		.addShortcuts("sb")
		...
		.setFunction(async(message, args, client) => {
			...
			if (!client.dp[user.id] || user.bot) return message.channel.send("That user is not a bot!");
			if (Math.random() > 0.3) return message.channel.send("Nah");
			await client.off("message", client.dp[user.id]);
			delete client.dp[user.id]
